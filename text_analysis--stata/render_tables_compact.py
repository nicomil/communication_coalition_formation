...
def render_compact_academic_table(data, col_headers, title, out_name, col_widths, figsize=(7.6, 8.4), note="Logit coefficients with group-clustered standard errors in parentheses. * p<0.10, ** p<0.05, *** p<0.01"):
    ...
    for (r, c), cell in tbl.get_celld().items():
        ...
        if r == 0:
            ...
            if r > n_rows - 5:
                cell.set_facecolor('#F6F8F6')
                if c == 0:
                    cell.set_text_props(weight='bold', color='#111111')
                else:
                    cell.set_text_props(color='#111111')
    
    # Title directly above table with minimal padding
    plt.title(title, fontsize=11.5, weight='bold', color='#007A33', pad=8, loc='left', x=0.02)
    
    # Footnote directly below table with minimal padding
    plt.figtext(0.03, 0.02, note, ha='left', fontsize=8, color='#555555', style='italic')
    
    png_path = os.path.join(pic_dir, f"{out_name}.png")
    pdf_path = os.path.join(pic_dir, f"{out_name}.pdf")
    
    plt.savefig(png_path, dpi=300, bbox_inches='tight', pad_inches=0.02, facecolor='white')
    plt.savefig(pdf_path, bbox_inches='tight', pad_inches=0.02, facecolor='white')
    plt.close()
    print(f"Generated compact: {png_path}")
...
headers_3m = ["Regressors", "(1)", "(2)", "(3)"]

render_compact_academic_table(
    data_3m, headers_3m,
