...
def render_compact_table_fixed(data, col_headers, out_name, col_widths, note_text):
    n_data_rows = len(data)
    
    # Compact figure geometry: 8.5 width x 7.5 height
    ...
    # Note immediately under table without gap
    ...
    note_cell.set_text_props(fontsize=8, color='#555555', style='italic', ha='left')
    ...
    plt.savefig(pdf_path, dpi=300, bbox_inches='tight', pad_inches=0.02, facecolor='white')
    plt.close()
    print(f"Rendered compact: {png_path}")

# Perfect column proportions: Column 0 = 0.28, Columns 1-6 = 0.12 each (sum = 1.0)
render_compact_table_fixed(
