...
# Define elegant rendering function
def render_academic_table(data, col_headers, title, out_name, col_widths, figsize=(8.5, 9.8), note="Logit coefficients with group-clustered standard errors in parentheses. * p<0.10, ** p<0.05, *** p<0.01"):
    ...
    for (r, c), cell in tbl.get_celld().items():
        cell.set_edgecolor('white') # no vertical gridlines
        cell.set_linewidth(0)
        
        # Header formatting
        if r == 0:
            cell.set_facecolor('#F2F5F3')
            cell.set_text_props(weight='bold', color='#005222' if c==0 else '#111111', ha='left' if c==0 else 'center')
            cell.set_edgecolor('#007A33')
            cell.set_linewidth(1.5)
        else:
            # Alternate or white background
            ...
            if r > 0 and r % 2 == 0 and r <= 24:
                cell.set_text_props(color='#555555', size=8.5)
    ...
    ax.axhline(y=0.96, color='#007A33', linewidth=2.0, xmin=0.03, xmax=0.97)
    ax.axhline(y=0.90, color='#007A33', linewidth=1.0, xmin=0.03, xmax=0.97)
    ax.axhline(y=0.19, color='#CCCCCC', linewidth=1.0, xmin=0.03, xmax=0.97)
    ax.axhline(y=0.07, color='#007A33', linewidth=1.5, xmin=0.03, xmax=0.97)
    
    plt.title(title, fontsize=12, weight='bold', color='#007A33', pad=12, loc='left', x=0.03)
    plt.figtext(0.04, 0.02, note, ha='left', fontsize=8, color='#666666', style='italic')
...
render_academic_table(
    data_3m, headers_3m,
    "Determinants of Persuasion: Key Logistic Specifications",
...
render_academic_table(
    data_ame, headers_3m,
    "Determinants of Persuasion: Average Marginal Effects (dy/dx)",
...
render_academic_table(
    data_6m, headers_6m,
    "Determinants of Persuasion: Full Logistic Specifications (1-6)",
